...

{
	...
	function QfeSql($pconnect=false,$nconnect=true)
	...
	function SetSource($host,$username,$pwd,$dbname,$dbprefix="qf_")
	...
	function GetClassId($id){
		...
		if(!$id){
			...
			return 'Error,you must have a $id to do this method!';

		}

		$this->Query("rs","SELECT * FROM #@__type WHERE CODE LIKE '%,$id,%'");

		if($rt=$this->GetArray("rs")){

			do{

				$class[] = $rt['id'];

			}while($rt=$this->GetArray("rs"));
...

	{
		...
		$emsg .= "<div><a href='http://www.myqingfeng.cn' target='_blank' style='color:red'>Technical Support: http://www.myqingfeng.cn</a></div>";
...

{
	...
	$log_file = QFINC.'/../data/'.md5($cfg_cookie_encode).'_safe.txt';
